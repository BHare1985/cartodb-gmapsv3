...
CartoDBInfowindow.prototype.setContent = function(content) {
	if (this.div_) {
		var div = this.div_, top = this.getElementsByClassName("top", div)[0];
...
CartoDBInfowindow.prototype.setPosition = function(latlng) {
	...
	if (this.div_) {
		var div = this.div_, pixPosition = this.getProjection().fromLatLngToDivPixel(this.latlng_);
		if (pixPosition) {
...
CartoDBInfowindow.prototype._adjustPan = function() {
	var left = 0, top = 0, pixPosition = this.getProjection().fromLatLngToContainerPixel(this.latlng_), container = this.map_.getDiv(), div_height = this.div_.clientHeight;
...
!function(a) {
	function A(a, b) {
		...
		var c = { duration: b.duration, easing: b.easing, after: b.after };
	...
	function y(a, b, c) {
		var d = [], f = [], i = c.duration || 1e3, j = c.easing || "ease-out", k = "";
		i = i + "ms", a.addEventListener(l, function m() { a.setAttribute("style", k), c.after && c.after(), a.removeEventListener(l, m, !0) }, !0), setTimeout(function() {
			var c;
	...
	function x(a, b, c, d) {
		c = c || {};
		var e = w(b), f = a.currentStyle ? a.currentStyle : getComputedStyle(a, null), g = {}, h = +(new Date), i, j = c.duration || 200, k = h + j, l, m = c.easing || function(a) { return-Math.cos(a * Math.PI) / 2 + .5 };
	...
	function s(a, b, c) { return(a + (b - a) * c).toFixed(3) }
